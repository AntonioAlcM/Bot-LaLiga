...
def recolectDataSegunda():
	"""Funcion que scrapea de la web la información de la clasificación de la liga de 2ª division española de fútbol.
	...
	# Comprobamos que la petición nos devuelve un Status Code = 200
		...
		for i in range(len(v_team)):
			fun_bd.insertClasificacionSegunda(v_team[i],v_point[i],i+1)
...
def selectDataClasiSeg():
	"""Funcion que obtiene los datos de la clasificación de 2ª guardados en la base de datos y los convierte en un string para su posterior envio.

	"""
	result=fun_bd.selectClasificacionSegunda()
	...
	msn2=msn2.replace("xc3xa9","é")
	msn2=msn2.replace("xc3xa1","á")
	msn2=msn2.replace("xc3xa0","á")
	msn2=msn2.replace("xc3xb3","ó")
	msn2=msn2.replace("xc3xad","í")
	
	return msn2

def inserDataResultSegunda():
	"""Funcion que scrapea de la web la información de los resultados de la jornada de la liga de 2ª division española de fútbol.

	"""
	url = "https://resultados.as.com/resultados/futbol/segunda/jornada/"
	...
	if statusCode == 200:
		...
		if len(v_result)<11:
			for i in range(len(v_result),11):
				v_result.insert(i,"-")
		...
		for i in range(11):
			fun_bd.insertResultadosSegunda(v_tlocal[i],v_result[i],v_tvisit[i],i+1)
		
	else:
	    	print "Status Code %d" %statusCode
	    	
	
def selectDataResultSegunda():
	"""Funcion que obtiene los datos de los resultados de 2ª division guardados en la base de datos y los convierte en un string para su posterior envio.

	"""
	result=fun_bd.selectResultadosSegunda()
	result1=fun_bd.selectMarcadorSegunda()
	result2=fun_bd.selectVisitanteSegunda()
